...
func run() -> bool:
	...
	_expect_prompt("夏离逐渐国书化")
	controller.interact()
	_expect_flag("saw_xiali_fading")
	_move(Vector2i(1, 0), 1)
	_expect_prompt("夏离的私人锚点")
	controller.interact()
	_expect_flag("kept_xiali_private_anchor")
	_move(Vector2i(1, 0), 1)
	_move(Vector2i(0, -1), 1)
	_face(Vector2i(0, -1))
	_expect_prompt("绑定夏离")
	...
	_expect_prompt("远程学塾")
	controller.interact()
	_expect_flag("confirmed_remote_classrooms")
	_move(Vector2i(1, 0), 2)
	_move(Vector2i(0, -1), 1)
	_face(Vector2i(0, -1))
	...
	_expect_prompt("静默信号")
	controller.interact()
	_expect_flag("identified_silent_interference")

	_move(Vector2i(1, 0), 1)
	_face(Vector2i(0, -1))
	_expect_prompt("第一帧静默影像")
	controller.interact()
	_expect_flag("saw_first_silent_frame")
	...
	_expect_prompt("写：名")
	controller.interact()
	_expect_flag("named_silent_interference")
	_move(Vector2i(1, 0), 1)
	_face(Vector2i(0, -1))
	_expect_prompt("写：止")
	controller.interact()
	...
	controller.interact()
	controller.interact()
	_expect_flag("stabilized_silent_interference")
