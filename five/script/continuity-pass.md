...
第三幕的四个藏书选择现在不会卡死，且已能影响第四幕指标、第六幕支持度、关键交互文本和 HUD 目标文案。后续如果要让玩家在不读目标栏的情况下也能看见差异，还需要给 `chose_royal_books`、`chose_engineer_books`、`chose_parent_books` 增加地点陈设或视觉状态变化。
...
第五幕已经写了夏离逐渐国书化，并补了 `kept_xiali_private_anchor`：纪子轩用旧讲堂的椅子给夏离留一个私人锚点。`xiali_fading` 和 `bind_xiali` 现在强化了他的语言变化：他会先响应城市节点，再努力把“我们”改回“我”。后续视觉 pass 仍应给他单独的投影、延迟、多节点出现状态。
...
- 分支线：给第三幕路线差异补第四幕地点陈设或视觉状态，而不是只让 metrics、日志与目标栏变化。
- 夏离线：给第五幕国书化状态补投影、延迟和多节点视觉演出。
- 父母线：给最终结尾补真正面对面或后日谈式寻找结果。
